...
const QuizzesReducer = (state = initialState, action) => {
	switch (action.type) {
		case actionTypes.GET_ALL_QUIZZES:
			return state
                .withMutations(mutant => {
                    mutant.set('allQuizzes', new OrderedMap().withMutations(map => {
		...
		case actionTypes.FINISH_QUIZ_SUCCESS:
			return state
				.withMutations(mutant => {
					...
					mutant.set('isUnexpectedFinished', action.payload.isUnexpectedFinished ? true : false);
				});
		case actionTypes.RECEIVE_INCOMING_QUESTION:
			return state
				.withMutations(mutant => {
					mutant.set('activeQuestion', activeQuestionModel({
		...
		case actionTypes.ANSWER_QUESTION_SUCCESS:
			return state
				.withMutations(mutant => {
					mutant.setIn(['activeQuestion', 'question', 'userAnswer'], userAnswerModel({
